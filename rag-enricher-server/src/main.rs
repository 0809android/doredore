...
use rag_enricher_core::core::enricher::RAGEnricher;
...
async fn search(
    ...
    let rag = state.rag.lock().unwrap();
    match rag.search(&query.q, query.collection.as_deref(), None, top_k, threshold) {
        Ok(results) => {
...
async fn enrich(
    ...
    let rag = state.rag.lock().unwrap();
    match rag.enrich(&query.q, query.collection.as_deref(), None, top_k, 0.0) {
        Ok(result) => {
...
async fn main() -> anyhow::Result<()> {
    ...
    info!("Initializing RAG Enricher...");
    let rag = RAGEnricher::new(&db_path, Some(&model), None)?;
    info!("RAG Enricher initialized with model: {}", model);
