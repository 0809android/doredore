...
REST API server and Admin UI for RAG Enricher, built with Axum and Alpine.js.
...
```bash
# Run in debug mode
cargo run --package rag-enricher-server

# Run with logs
RUST_LOG=debug cargo run --package rag-enricher-server

# Format code
cargo fmt

# Lint
cargo clippy --package rag-enricher-server
```
